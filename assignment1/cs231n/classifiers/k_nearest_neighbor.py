...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i,:] = np.sqrt(np.sum((np.expand_dims(X[i,:], axis=0) - self.X_train)**2, axis=1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # sort the train indices by increasing distance to the test example i
            neigh_sorted_ind = np.argsort(dists[i,:])
            # k-nearest train neighbours labels
            closest_y = self.y_train[neigh_sorted_ind[:k]]
            ...
            # find the set of distinct neighbours labels
